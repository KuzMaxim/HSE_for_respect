...
async def plan_trip_service(query: str):
    ...
    prompt = (
        ...
        "Для этого рейса укажи только эти поля: date, airline, price (стоимость билетов туда-обратно), link (если не получится на рейс, то хотя бы на агрегатор)."
        "3. Подбери самый подходящий отель в этом городе на эти даты с этим бюджетом. "
        "Для этого отеля укажи только эти поля: name (название), location (адрес), price (цена за весь период), rating (от 0 до 5), features (список удобств), link (ссылка на агрегатор, а лучше на сам отель). "
        "Верни только JSON вида: {\"plan\":{\"city\":...,\"checkin\":...,\"checkout\":...,\"budget\":...}, "
        "\"hotel\": {\"name\":...,\"location\":...,\"price\":...,\"rating\":...,\"features\":[], \"link\":...}, "
        "\"flight\": {\"date\":...,\"airline\":...,\"price\":...,\"link\":...}}}"
        "\nЗапрос пользователя: " + query
    )
    data = {
        "contents": [
